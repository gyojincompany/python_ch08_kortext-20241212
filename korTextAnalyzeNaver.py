...
data = json.loads(open("data/인공지능_naver_news.json","r",encoding="utf-8").read())  # json 파일을 불러오기
print(data)
print(len(data))
...
for item in data:
    news_message = news_message + re.sub(r"[^\w]"," ", item["description"])
...
del word_count["인공"]  # 검색 단어는 삭제
del word_count["지능"]  # 검색 단어는 삭제
# del word_count["한국"]
# del word_count["선수"]
# del word_count["지난"]
